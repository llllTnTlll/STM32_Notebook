...
void SSD1315::ShowFrame(){
	if(SOURCE_NOW == FROM_USB){
		TransmitCMD(REQUEST_NEW_FREAM);
		RecvData(1024);
		memcpy(OLED_GRAM, RecvDataBuffer, sizeof(RecvDataBuffer));
		ResetBuffer();
	}
...
void SSD1315::USBSignalMode(bool isUSBMode){
	if(isUSBMode){
		//OLED操作
		SwitchAdrMode(PAGE_MODE);
		OLED_PrintASCIIString(1, 1, const_cast<char*>("Connecting..."), &afont16x8, OLED_COLOR_NORMAL);
		ShowFrame();
		...
		//OLED操作
		ClearFrame();
		OLED_PrintASCIIString(1, 1, const_cast<char*>("CONNECTED!!!"), &afont16x8, OLED_COLOR_NORMAL);
		ShowFrame();
		SOURCE_NOW = FROM_USB;
	}
	// 添加退出USB模式逻辑
